...
class ProjFourphp_Datum
{
    ...
    public function __construct($proj)
    {
        ...
        $this->datumType = ProjFourphp_Common::$pjdWgsEightFour;
//default setting
        ...
        if (isset($proj->datumParams)) {
            ...
            if ($proj->datumParams[0] != 0
                || $proj->datumParams[1] != 0
                || $proj->datumParams[2] != 0) {
                $this->datumType = ProjFourphp_Common::$pjdThreeParam;
            }

            if (sizeof($proj->datumParams) > 3) {
                if ($proj->datumParams[3] != 0
                    || $proj->datumParams[4] != 0 ||
                    $proj->datumParams[5] != 0
                    || $proj->datumParams[6] != 0) {
                    ...
                    $proj->datumParams[6] =
                        ($proj->datumParams[6] / 1000000.0) + 1.0;
                }
            }
    ...
    public function compare_datums($dest)
    {
        ...
        if ($this->datumType != $dest->datumType) {
            ...
            || abs($this->es - $dest->es) > 0.000000000050) {
            // the tolerence for es is to ensure that GRS80 and WGS84
            // are considered identical
            return false;
        } else if ($this->datumType == ProjFourphp_Common::$pjdThreeParam) {
        ...
        } else if ($this->datumType == ProjFourphp_Common::$pjdGridshift ||
            $dest->datumType == ProjFourphp_Common::$pjdGridshift) {
    ...
    public function geodeticToGeocentric($p)
    {
        ...
        } else if ($latitude > ProjFourphp_Common::$halfPi
            && $latitude < 1.001 * ProjFourphp_Common::$halfPi) {
            $latitude = ProjFourphp_Common::$halfPi;
        } else if (($latitude < - ProjFourphp_Common::$halfPi)
            || ($latitude > ProjFourphp_Common::$halfPi)) {
            /* Latitude out of range */
            ProjFourphp::reportError('geocent:lat out of range:' . $latitude);
            return null;
        }
        ...
        $rn        = $this->a / (sqrt(1.0e0 - $this->es * $sinTwoLat));
    ...
    public function geocentricToGeodetic($pC)
    {
         ...
         * -Latitude(iter-1))
          ...
          $iter;        // of continous iteration, max. 30 is always enough
         * (s.a.)
          $Longitude;
          $Latitude;
          $height;
        ...
        do {
            ...
            $height = $p * $cphio + $z * $sphio - $rn *
                (1.0 - $this->es * $sphio * $sphio);
