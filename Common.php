...
class Proj4php_Common
{
    ...
    public $halfPi = 1.570796326794896619; //Math.PI*0.5,
    public $twoPi = 6.283185307179586477; //Math.PI*2,
    public $fortPi = 0.78539816339744833;
    public $rToD = 57.29577951308232088;
    public $dToR = 0.01745329251994329577;
    public $secToRad = 4.84813681109535993589914102357e-6; /* SEC_TO_RAD = Pi/180/3600 */
    public $epsln = 1.0e-10;
    public $maxIter = 20;
    // following constants from geocent.c
    public $cosOf67P5 = 0.38268343236508977;  /* cosine of 67.5 degrees */
    public $adC = 1.0026000;                /* Toms region 1 constant */

    /* datum_type values */
    public $pjdUnknown = 0;
    public $pjd3Param = 1;
    public $pjd7Param = 2;
    public $pjdGridshift = 3;
    public $pjdWgs84 = 4;   // WGS84 or equivalent
    public $pjdNodatum = 5;   // WGS84 or equivalent
...
// -----------------------------------------------------------------
    public function tsfnz($eccent, $phi, $sinphi)
    {
        ...
        return (tan(.5 * ($this->halfPi - $phi)) / $con);
    }
    ...
    {
        $eccnth = .5 * $eccent;
        $phi = $this->halfPi - 2 * atan($ts);
        for ($i = 0; $i <= 15; $i++) {
            $con = $eccent * sin($phi);
            $dphi = $this->halfPi - 2 * atan($ts * (pow(((1.0 - $con) / (1.0 + $con)), $eccnth))) - $phi;
            $phi += $dphi;
...
// Function to adjust longitude to -180 to 180; input in radians
    ...
    {
        $x = (abs($x) < $this->pi) ? $x : ($x - ($this->sign($x) * $this->twoPi) );
        return $x;
    }
...
// Function to adjust latitude to -90 to 90; input in radians
    ...
    {
        $x = (abs($x) < $this->halfPi) ? $x : ($x - ($this->sign($x) * $this->pi) );
        return $x;
    }
...
// Latitude Isometrique - close to tsfnz ...
    ...
    {
        if (abs($phi) > $this->halfPi)
            return +NaN;
        if ($phi == $this->halfPi)
            return INF;
        if ($phi == -1.0 * $this->halfPi)
            return -1.0 * INF;

        $con = $eccent * $sinphi;
        return log(tan(($this->halfPi + $phi) / 2.0)) + $eccent * log((1.0 - $con) / (1.0 + $con)) / 2.0;
    }

    public function fL($x, $L)
    {
        return 2.0 * atan($x * exp($L)) - $this->halfPi;
    }
...
// Inverse Latitude Isometrique - close to ph2z
    ...
    public function pj_inv_mlfn($arg, $es, $en)
    {
        $k = 1. / (1. - $es);
        $phi = $arg;
        for ($i = Proj4php::$common->maxIter; $i; --$i) { /* rarely goes over 2 iterations */
            $s = sin($phi);
            $t = 1. - $es * $s * $s;
