...
class ProjFourphp_ProjMerc
{
    ...
    public function init()
    {
        ...
        //$this->e = sqrt( $this->es );
        //?$this->m1 = cos($this->lat_origin) / (sqrt( 1.0 - $this->es *
        // sin($this->lat_origin) * sin($this->lat_origin)));
        ...
        if ($this->latTs) {
            if ($this->sphere) {
                ...
                $this->kZero =
                    ProjFourphp_Common::msfnz($this->es, sin($this->latTs), cos($this->latTs));
            }
        }
    }
    ...
    public function forward($p)
    {
        ...
        if (abs(abs($lat) - ProjFourphp_Common::$halfPi)
            ...
            if ($this->sphere) {
                $x = $this->xZero + $this->a * $this->kZero
                    * ProjFourphp_Common::adjustLon($lon - $this->longZero);
                $y = $this->yZero + $this->a * $this->kZero
                    * log(tan(ProjFourphp::$common->fortPi + 0.5 * $lat));
            } else {
                $sinphi = sin(lat);
    ...
    public function inverse($p)
    {
        ...
        if ($this->sphere) {
            $lat = ProjFourphp_Common::$halfPi - 2.0
                * atan(exp(-$y / $this->a * $this->kZero));
        } else {
        ...
        $lon =
            ProjFourphp_Common::adjustLon($this->longZero + $x / ($this->a * $this->kZero));
